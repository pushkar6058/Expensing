...
function StatBlock({ label, value, tone = 'text-white' }) {
  return (
    <div className='bg-zinc-900 border border-zinc-700 rounded-3xl p-4 sm:p-6'>
      <p className='text-sm sm:text-lg text-zinc-400'>{label}</p>
...
export default function TripAnalysisPage() {
  ...
  if (isLoading && !trip) {
    return (
      <div className='min-h-screen bg-black text-white px-4 py-6 sm:px-8 md:px-10 md:py-10 overflow-x-hidden flex items-center justify-center'>
        <p className='text-lg sm:text-2xl text-zinc-400'>Loading analysis...</p>
      </div>
    );
  }
  ...
  if (!trip) {
    return (
      <div className='min-h-screen bg-black text-white px-4 py-6 sm:px-8 md:px-10 md:py-10 overflow-x-hidden'>
        <div className='flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-zinc-700 pb-6'>
          <h1 className='text-3xl sm:text-5xl font-bold'>Trip Analysis</h1>
          <Link className='text-lg sm:text-2xl text-emerald-400 hover:underline' to='/group-trips'>
            Back to trips
          </Link>
        </div>
        <p className='text-lg sm:text-2xl text-zinc-300 mt-10'>Trip not found.</p>
      </div>
    );
  }
  ...
  return (
    <div className='min-h-screen bg-black text-white px-4 py-6 sm:px-8 md:px-10 md:py-10 overflow-x-hidden'>
      <div className='flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between border-b border-zinc-700 pb-6'>
        <div>
          <Link className='text-lg sm:text-2xl text-emerald-400 hover:underline' to={`/group-trips/${tripId}`}>
            Back to trip
          </Link>
          <h1 className='text-3xl sm:text-5xl md:text-6xl font-bold mt-3 sm:mt-4 break-words'>{trip.title} Analysis</h1>
          <p className='text-lg sm:text-2xl text-zinc-300 mt-2 sm:mt-3'>{trip.date}</p>
        </div>
      </div>
      ...
      {currentUserSummary && (
        <div className='grid md:grid-cols-3 gap-4 sm:gap-6 mt-4 sm:mt-6'>
          <StatBlock label='You Paid' value={formatMoney(currentUserSummary.contribution)} tone='text-emerald-400' />
          <StatBlock
            ...
            tone={currentUserSummary.balance >= 0 ? 'text-emerald-400' : 'text-orange-400'}
          />
        ...
        <div className='bg-zinc-900 border border-zinc-700 rounded-3xl p-4 sm:p-6'>
          ...
          <div className='mt-6 space-y-4'>
            {analysis.memberSummaries.map((entry) => (
              <div key={entry.memberId} className='bg-zinc-800 rounded-2xl p-4 sm:p-5'>
                <div className='flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between'>
                  <div>
                    <p className='text-lg sm:text-2xl font-semibold break-words'>{entry.member.name}</p>
                    <p className='text-sm text-zinc-400'>{entry.member.email}</p>
                  </div>
                  <p className={`text-lg sm:text-2xl font-bold break-words ${entry.balance >= 0 ? 'text-emerald-400' : 'text-orange-400'}`}>
                    {entry.balance >= 0 ? '+' : '-'}{formatMoney(Math.abs(entry.balance))}
                  </p>
                </div>
                <div className='grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4 mt-4 text-sm sm:text-lg text-zinc-300'>
            ...
            {outgoingSettlements.length ? (
              <div className='mt-6 space-y-3'>
                {outgoingSettlements.map((entry, index) => (
                  <div key={`${entry.fromId}-${entry.toId}-${index}`} className='bg-zinc-800 rounded-2xl p-4'>
                    <p className='text-base sm:text-2xl break-words'>Pay <span className='font-semibold text-white'>{entry.to.name}</span> {formatMoney(entry.amount)}</p>
                  </div>
                ))}
              </div>
            ) : (
              <p className='text-base sm:text-xl text-zinc-400 mt-6'>You do not owe anyone.</p>
            )}
          </div>
            ...
            {incomingSettlements.length ? (
              <div className='mt-6 space-y-3'>
                {incomingSettlements.map((entry, index) => (
                  <div key={`${entry.toId}-${entry.fromId}-${index}`} className='bg-zinc-800 rounded-2xl p-4'>
                    <p className='text-base sm:text-2xl break-words'>Get <span className='font-semibold text-white'>{formatMoney(entry.amount)}</span> from {entry.from.name}</p>
                  </div>
                ))}
              </div>
            ) : (
              <p className='text-base sm:text-xl text-zinc-400 mt-6'>No one owes you money.</p>
            )}
          </div>
                ...
                {analysis.settlements.map((entry, index) => (
                  <div key={`${entry.fromId}-${entry.toId}-${index}`} className='bg-zinc-800 rounded-2xl p-4'>
                    <p className='text-sm sm:text-xl break-words'>
                      <span className='font-semibold text-white'>{entry.from.name}</span> pays{' '}
                      <span className='font-semibold text-white'>{entry.to.name}</span> {formatMoney(entry.amount)}
                    </p>
                  </div>
                ))}
              </div>
            ) : (
              <p className='text-base sm:text-xl text-zinc-400 mt-6'>Everything is already balanced.</p>
            )}
          </div>
        </div>
      </div>
