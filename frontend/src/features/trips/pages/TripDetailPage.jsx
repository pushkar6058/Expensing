  ...
  if (isLoading) {
    return (
      <div className='min-h-screen bg-black text-white px-4 py-6 sm:px-8 md:px-10 md:py-10 overflow-x-hidden flex items-center justify-center'>
        <p className='text-lg sm:text-2xl text-zinc-400'>Loading...</p>
      </div>
    );
  }
    ...
    <div className='min-h-screen bg-black text-white px-4 py-6 sm:px-8 md:px-10 md:py-10 overflow-x-hidden'>
      <div className='flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between border-b border-zinc-700 pb-6'>
        <div>
          ...
          <p className='text-sm sm:text-xl text-zinc-500 mt-3 sm:mt-4 break-all'>Trip ID: {trip._id}</p>
        </div>

        <div className='w-full sm:min-w-[260px] sm:w-auto bg-zinc-900 border border-zinc-700 rounded-3xl p-4 sm:p-6'>
          <p className='text-lg sm:text-2xl text-zinc-300'>Budget</p>
          <p className='text-2xl sm:text-4xl font-bold mt-2 break-words'>{formatMoney(trip.budget)}</p>
          <p className='text-lg sm:text-2xl text-zinc-300 mt-4 sm:mt-5'>Spent</p>
          <p className='text-2xl sm:text-4xl font-bold mt-2 break-words'>{formatMoney(trip.spent)}</p>
          <p className='text-lg sm:text-2xl text-zinc-300 mt-4 sm:mt-5'>Status</p>
      ...
      <div className='grid lg:grid-cols-3 gap-4 sm:gap-6 mt-8 sm:mt-10'>
        <div className='lg:col-span-1 bg-zinc-900 border border-zinc-700 rounded-3xl p-4 sm:p-6'>
          ...
          <form className='mt-6 space-y-4' onSubmit={handleAdd}>
            <input
              ...
              className='w-full p-4 rounded-2xl bg-zinc-800 text-base sm:text-lg'
            />
            <input
              type='number'
              min='0'
        ...
        <div className='lg:col-span-2 bg-zinc-900 border border-zinc-700 rounded-3xl p-4 sm:p-6'>
          <div className='mb-8'>
            <div className='flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between'>
              <h2 className='text-2xl sm:text-4xl font-bold'>Members</h2>
              <p className='text-base sm:text-2xl text-zinc-300'>{trip.members?.length || 0} joined</p>
            </div>
            ...
            {trip.members?.length ? (
              <div className='mt-6 flex flex-wrap gap-3'>
                {trip.members.map((member) => (
                  <div
                    ...
                    className='px-4 py-3 rounded-2xl bg-zinc-800 border border-zinc-700 max-w-full'
                  >
              ...
              <p className='text-lg sm:text-2xl text-zinc-300 mt-6'>No members yet.</p>
            )}
          </div>

          <div className='flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between'>
            <h2 className='text-2xl sm:text-4xl font-bold'>Expenses</h2>
            <p className='text-base sm:text-2xl text-zinc-300'>{expenses.length} items</p>
          </div>

          {expenses.length === 0 ? (
            <p className='text-lg sm:text-2xl text-zinc-300 mt-8'>No expenses yet.</p>
          ) : (
            <div className='mt-8 space-y-4'>
              {expenses.map((expense) => (
                <div
                  key={expense._id || expense.id}
                  className='flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between bg-zinc-800 rounded-2xl p-4 sm:p-5'
                >
                  <div>
                    <p className='text-xl sm:text-3xl font-semibold break-words'>{expense.description}</p>
                    <p className='text-sm sm:text-lg text-emerald-400 mt-1 break-words'>by {expense.createdBy?.name || 'Unknown user'}</p>
                    <p className='text-sm sm:text-xl text-zinc-400 mt-1'>
                  ...
                  <div className='flex flex-wrap items-center gap-3 sm:gap-4'>
                    ...
                    {(expense.createdBy?._id || expense.createdBy?.id) === (currentUser?._id || currentUser?.id) && (
                      <button
                        ...
                        className='px-4 sm:px-5 py-3 rounded-2xl border border-zinc-600 text-base sm:text-2xl hover:bg-zinc-700'
                      >
                        Delete
                      </button>
                    )}
